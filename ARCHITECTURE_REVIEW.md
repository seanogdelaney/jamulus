...
|---|---|---|---|
| A. identity / ownership | **confirmed** | `src/server.h:CServer` owns `vecChannels`, `src/server.cpp:CServer::FindChannel`, `InitChannel`, `FreeChannel`, `CreateChannelList`, `OnTimer` | Split the one index into a session-slot pool (`CChannel`) and a visible-source pool (`CServerSource`). The configured max limits physical user sessions, while visible sources use the fixed compile-time pool. |
...
| H. bounds / scope | **confirmed** | `src/global.h:MAX_NUM_IN_OUT_CHANNELS=64`, `MAX_NUM_CHANNELS=150`, `MAX_SIZE_BYTES_NETW_BUF=20000`; `src/protocol.cpp` split-message implementation | Client rows are bounded by `MAX_NUM_IN_OUT_CHANNELS`; physical user sessions use the configured server limit and visible sources use the `MAX_NUM_CHANNELS` pool. Frames use a 1200-byte application UDP limit, fixed maximum record payload of 515 bytes (3-byte record header + 512-byte stereo Raw frame), and at most 32 fragments for 64 rows. |
