...
|---|---|---|---|
| A. identity / ownership | **confirmed** | `src/server.h:CServer` owns `vecChannels`, `src/server.cpp:CServer::FindChannel`, `InitChannel`, `FreeChannel`, `CreateChannelList`, `OnTimer` | Split the one index into a session-slot pool (`CChannel`) and a visible-source pool (`CServerSource`). `--numchannels` limits visible sources; `--numclient` independently limits physical user sessions. |
...
| H. bounds / scope | **confirmed** | `src/global.h:MAX_NUM_IN_OUT_CHANNELS=64`, `MAX_NUM_CHANNELS=150`, `MAX_SIZE_BYTES_NETW_BUF=20000`; `src/protocol.cpp` split-message implementation | Client rows are bounded by `MAX_NUM_IN_OUT_CHANNELS`; `--numchannels` bounds visible faders and `--numclient` bounds physical sessions, both within the `MAX_NUM_CHANNELS` fixed pool. Frames use a 1200-byte application UDP limit, fixed maximum record payload of 515 bytes (3-byte record header + 512-byte stereo Raw frame), and at most 32 fragments for 64 rows. |
