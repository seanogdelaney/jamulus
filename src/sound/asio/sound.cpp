...
int CSound::Init ( const int iNewPrefMonoBufferSize )
{
    ASIOMutex.lock(); // get mutex lock
    {
        ...
        vecInputAudioSndCrd.Init ( iASIOBufferSizeMono * lNumInChanPlusAddChan );
        PrepareCapturedInputAudio ( iASIOBufferSizeMono, lNumInChanPlusAddChan );
...
void CSound::bufferSwitch ( long index, ASIOBool )
{
    ...
    {
        // CAPTURE -------------------------------------------------------------
        CVector<int16_t>& vecInputAudioSndCrd = pSound->vecInputAudioSndCrd;
        for ( int i = 0; i < pSound->lNumInChanPlusAddChan; i++ )
        {
            int iSelCH, iSelAddCH;

            GetSelCHAndAddCH ( i, pSound->lNumInChan, iSelCH, iSelAddCH );
            ...
            case ASIOSTInt16LSB:
            {
                ...
                {
                    vecInputAudioSndCrd[pSound->lNumInChanPlusAddChan * iCurSample + i] = pASIOBuf[iCurSample];
                }
            ...
            case ASIOSTInt24LSB:
                for ( iCurSample = 0; iCurSample < iASIOBufferSizeMono; iCurSample++ )
                {
                    ...
                    vecInputAudioSndCrd[pSound->lNumInChanPlusAddChan * iCurSample + i] = static_cast<int16_t> ( iCurSam );
                }
            ...
            case ASIOSTInt32LSB:
            {
                ...
                for ( iCurSample = 0; iCurSample < iASIOBufferSizeMono; iCurSample++ )
                {
                    vecInputAudioSndCrd[pSound->lNumInChanPlusAddChan * iCurSample + i] = static_cast<int16_t> ( pASIOBuf[iCurSample] >> 16 );
                }
...
// NOT YET TESTED
                ...
                for ( iCurSample = 0; iCurSample < iASIOBufferSizeMono; iCurSample++ )
                {
                    ...
                    vecInputAudioSndCrd[pSound->lNumInChanPlusAddChan * iCurSample + i] = Flip16Bits ( static_cast<int16_t> ( iCurSam ) );
                }
                break;
...
// NOT YET TESTED
                ...
                for ( iCurSample = 0; iCurSample < iASIOBufferSizeMono; iCurSample++ )
                {
                    ...
                    vecInputAudioSndCrd[pSound->lNumInChanPlusAddChan * iCurSample + i] = static_cast<int16_t> (
...
// NOT YET TESTED
        ...
        // Keep the legacy selected stereo pair as the callback/output buffer.
        for ( iCurSample = 0; iCurSample < iASIOBufferSizeMono; ++iCurSample )
        {
            vecsMultChanAudioSndCrd[2 * iCurSample] =
                vecInputAudioSndCrd[pSound->lNumInChanPlusAddChan * iCurSample + pSound->vSelectedInputChannels[0]];
            vecsMultChanAudioSndCrd[2 * iCurSample + 1] =
                vecInputAudioSndCrd[pSound->lNumInChanPlusAddChan * iCurSample + pSound->vSelectedInputChannels[1]];
        }
        pSound->SetCapturedInputAudio ( vecInputAudioSndCrd, pSound->lNumInChanPlusAddChan );
