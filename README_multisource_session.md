...
* `--numchannels` caps visible source faders, preserving the original channel-limit
  meaning. `--numclient` independently caps physical user sessions; when omitted it
  defaults to the `--numchannels` value. Reservations are atomic and hidden.
